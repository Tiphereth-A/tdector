//! UTF-8 inputs and fully staged, atomic file commits for the CLI adapter.
...
impl std::error::Error for IoError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Self::OutputExists { .. } | Self::InputChanged { .. } => None,
        }
    }
}
...
/// Read a file, or stdin for `-`, without transforming the original bytes.
///
/// The caller validates the number of stdin consumers before calling this.
/// Retaining these bytes also allows an in-place save to detect observed edits.
...
///
/// A missing output cannot alias the existing input. Other errors are preserved
/// instead of treating an unreadable destination as a safely distinct file.
...
/// Write an already generated artifact to stdout and observe flush failures.
///
/// The caller must not print a report on stdout when writing an artifact.
...
///
/// Existing symlinks are followed when overwriting, so an in-place edit retains
/// the link and replaces its target. Unix file permissions are preserved.
/// The original destination is never explicitly removed. `tempfile::persist`
/// uses a platform replacement operation (rename on Unix, `MoveFileExW` on
/// Windows); a failed commit drops only the temporary file.
///
/// For an in-place edit, `expected_input` supplies the originally loaded bytes.
/// They are compared after staging, immediately before committing. This detects
/// observed changes but does not lock out a concurrent writer after the check.
pub fn atomic_write(
    output: &Path,
    bytes: &[u8],
    overwrite: bool,
    ...
    // Canonicalize an existing target to place the staging file on the target's
    // filesystem, including when the final path component is a symlink.
    ...
    staged.flush()?;
    // On Windows, tempfile's replacement restores normal file attributes. Do
    // not make the stage read-only before committing: that would prevent its
    // cleanup when the subsequent input comparison refuses the write.
    #[cfg(unix)]
    if metadata.is_some() {
        staged
            .as_file()
    ...
    let committed = if overwrite {
        ...
        // Unlike an existence check followed by rename, this also refuses a
        // destination created by another process after the initial check.
...
mod tests {
    ...
    fn file_reads_retain_the_loaded_bytes() {
        ...
        assert_eq!(
            read_input(&input).expect("read original input bytes"),
            expected
        );
    }
