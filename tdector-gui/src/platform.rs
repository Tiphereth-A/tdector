...
impl FileIO {
    ...
    pub async fn save_file(
        ...
        Self::save_file_with_path(content, filename, filter_name, extensions)
            .await
            .map(|_| ())
    }

    /// Save through a GUI dialog and return its native destination when available.
    pub async fn save_file_with_path(
        content: &[u8],
        filename: &str,
        filter_name: &str,
        extensions: &[&str],
        ...
        match dialog.save_file().await {
            Some(handle) => {
                ...
                #[cfg(not(target_arch = "wasm32"))]
                let path = Some(handle.path().to_string_lossy().into_owned());
                #[cfg(target_arch = "wasm32")]
                let path = None;
                Ok(path)
            }
