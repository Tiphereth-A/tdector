...
pub fn render_clickable_tokens(
    ...
    ui.horizontal(|ui| {
        ...
        for token in tokens {
            ...
            let gloss_text = if gloss.is_empty() { "" } else { gloss };

            ui.vertical(|ui| {
                ui.add(
                    egui::Label::new(
                        egui::RichText::new(gloss_text)
    ...
    ui.allocate_ui_with_layout(
        ...
        |ui| {
            ...
            egui::Frame::none()
                ...
                .rounding(constants::GLOSS_BOX_ROUNDING)
                .show(ui, |ui| {
                    if dictionary_mode {
            ...
            let layout_job = create_highlighted_layout(
                &token.original,
                highlight,
                token_font_id,
                text_color
...
fn render_translation_box(
    ...
    egui::Frame::none()
        ...
        .rounding(constants::TRANSLATION_BOX_ROUNDING)
        .show(ui, |ui| {
            let text_color = if ui.visuals().dark_mode {
                colors::FONT_DARK
            } else {
