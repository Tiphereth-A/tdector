    ...
    if let Some(parent) = path.parent() {
        if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
            for ext in font_extensions {
                let font_file = parent.join(format!("{}.{}", stem, ext));
                if font_file.exists() {
                    font_path = font_file.to_str().map(|s| s.to_string());
                    break;
                }
            }
        }
...
fn convert_from_saved_project(saved: SavedProject) -> Option<Project> {
...
fn convert_to_saved_project(project: &Project) -> SavedProject {
    ...
    let sentences: Vec<SavedSentence> = project
        ...
        .map(|segment| {
            let words: Vec<usize> = segment
                .tokens
                .iter()
                .filter_map(|t| word_to_idx.get(&t.original).copied())
                .collect();
    ...
    SavedProject {
        ...
        font_path: project.font_path.clone(),
        vocabulary,
        sentences,
    }
}
